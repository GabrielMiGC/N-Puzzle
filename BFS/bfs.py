...
import resource  # Importa o módulo resource para medir o uso de memória
...
def generate_neighbors(state):
    ...
    print(f"Estado atual: {state}, Vizinhos: {neighbors}")  # Imprime o estado atual e seus vizinhos
...
def bfs(graph, start, goal):
    ...
    while queue:
        ...
        if state not in visited:
            ...
            if state == goal:
                ...
                mem_usage = resource.getrusage(resource.RUSAGE_SELF).ru_maxrss / 1024  # Obtém o uso de memória em kilobytes
                return path, (end_time - start_time), mem_usage  # Retorna o caminho, tempo de execução e uso de memória
            if state in graph:
    ...
    end_time = time.time()  # Registra o tempo de fim
    mem_usage = resource.getrusage(resource.RUSAGE_SELF).ru_maxrss / 1024  # Obtém o uso de memória em kilobytes
    return None, (end_time - start_time), mem_usage  # Retorna None se a solução não for encontrada, tempo de execução e uso de memória